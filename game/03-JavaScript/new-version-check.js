window.checkNewVersion = function() {
	const API_key = "AIzaSyCu0F1pUOocM6R0wJ4tmug6SPTX19JRVQc"; // Google API key, restricted to Blogspot
	const dol_blog_id = "7051760000701686365"; // Blogspot BlogID

	if (State.variables.notifyUpdate == undefined)
		State.variables.notifyUpdate = true;

	if (State.variables.notifyUpdate) {
		// get the list of last 10 posts and check their titles to find latest version
		$.ajax({
			url: "https://www.googleapis.com/blogger/v3/blogs/" + dol_blog_id + "/posts?key=" + API_key,
			dataType: "jsonp",
...
function handleVersionResponse(response) {
	const titleRE = new RegExp("Degrees of Lewdity\\.*? version (\\d+\\.\\d+(?:\\.\\d+(?:\\.\\d+)?)?)");
	...
	if ("items" in response) {
		...
		if (version && postDate) {
			let hours = Math.floor((Date.now() - postDate) / 1000 / 60 / 60); // hours since latest release
			let days = Math.floor(hours / 24); // whole days since latest release
