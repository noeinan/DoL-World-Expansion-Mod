...
Macro.add("doubleslider", {
	handler: function () {
		...
		if (this.args.length === 3){ //defaults
			leftcolor = "#fa0000";
			middlecolor = "#ab00fa";
			rightcolor = "#006cfa";
		}
		function updateValues(data, where){
			...
			var bar2 = data.slider.find('.irs-bar2');
			if (bar2.length === 0) bar2 = $('<span>').addClass('irs-bar2').appendTo(data.slider);
			bar2.css('width',''+(data.from/(data.max-data.min))*100+'%');

			var middleLabel = data.slider.find('.irs-single');
			var rightLabel = data.slider.find('.irs-to');
			var leftLabel = data.slider.find('.irs-from');
		...
		let input = $("<input>") /* create new element and wrap in jQuery */
		...
		let oldDrawHandles = slider.drawHandles;
