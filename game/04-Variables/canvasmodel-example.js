...
Renderer.CanvasModels["sexdemo"] = {
	 ...
	 */
	preprocess(options) {
		if (!options.xray_vaginal_speed) { // "" means use global
			options.xray_vaginal_speed = options.animation_speed;
		}
		if (!options.close_vagina_speed) { // "" means use global
			options.close_vagina_speed = options.animation_speed;
		}
		if (options.worn_under_lower) {
			let record = setup.colours.clothes_map[options.worn_under_lower_colour];
			if (record) {
	...
	layers: {
		"xray_vaginal": {
			...
			srcfn(options) {
				let cum = options.vagina_cumming ? "cum" : "";
		...
		"close_vagina_penis": {
			...
			srcfn(options) {
				return "img/sex/close/"+options.position+"/" +
					(options.balls ? "futavagina.png" : "futavaginanoballs.png")
			},
			filters: [ "close_vagina" ],
			animationfn(options) {
				return "sex-6f-"+options.close_vagina_speed;
		...
		"close_vagina_panties": {
			z: 253,
			showfn(options) {
				return options.close_vagina_show && !!options.worn_under_lower &&
					options.worn_under_lower_state === "totheside"
			},
			srcfn(options) {
